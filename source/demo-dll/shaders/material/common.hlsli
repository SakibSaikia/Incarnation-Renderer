...
#define __MATERIAL_COMMON_HLSLI_
	...
	#define TEX_SAMPLE(t,s,uv, ddx, ddy) (t.SampleLevel(s, uv, 0))
#elif CUSTOM_UV_DERIVATIVES
	#define TEX_SAMPLE(t,s,uv, ddx, ddy) (t.SampleGrad(s, uv, ddx, ddy))
#else
	#define TEX_SAMPLE(t,s,uv, ddx, ddy) (t.Sample(s,uv))
#endif
...
FMaterialProperties EvaluateMaterialProperties(FMaterial mat, float2 uv, SamplerState s, float2 ddx, float2 ddy)
{
...
//#endif
	...
	if (mat.m_emissiveTextureIndex != -1)
	{
		...
		output.emissive *= TEX_SAMPLE(emissiveTex, s, uv, ddx, ddy).rgb;
	}
	...
	if (mat.m_baseColorTextureIndex != -1)
	{
		...
		output.basecolor *= TEX_SAMPLE(baseColorTex, s, uv, ddx, ddy).rgb;
	}
	...
	if (output.bHasNormalmap)
	{
		...
		float2 normalXY = TEX_SAMPLE(normalmapTex, s, uv, ddx, ddy).rg;
	...
	if (mat.m_metallicRoughnessTextureIndex != -1)
	{
		...
		float2 metallicRoughnessMap = TEX_SAMPLE(metallicRoughnessTex, s, uv, ddx, ddy).rg;
	...
	if (mat.m_aoTextureIndex != -1)
	{
		...
		output.ao = TEX_SAMPLE(aoTex, s, uv, ddx, ddy).r;
	}
	...
	if (mat.m_transmissionTextureIndex != -1)
	{
		...
		output.transmission *= TEX_SAMPLE(transmissionTex, s, uv, ddx, ddy).r;
	}
	...
	if (mat.m_clearcoatTextureIndex != -1)
	{
		...
		output.clearcoat *= TEX_SAMPLE(clearcoatTex, s, uv, ddx, ddy).r;
	}
	...
	if (mat.m_clearcoatRoughnessTextureIndex != -1)
	{
		...
		output.clearcoatRoughness *= TEX_SAMPLE(clearcoatRoughnessTex, s, uv, ddx, ddy).r;
	}
